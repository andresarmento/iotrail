...
Mesmo layout binário do test_0/test_1 (little-endian, sem padding — espelha
o RecordHeader `#pragma pack(1)` do lado C++). A novidade do test_2 é só na
gravação (sync_interval/fsync); o formato do registro não muda:
    timestamp_ms : uint64
    topic_len    : uint16
    payload_len  : uint32
...
DEFAULT_PATH = Path(__file__).resolve().parent.parent.parent / "build" / "test_2" / "test_segment.bin"
